...
class MySQLClient(object):
	...
	conn_str = 'mysql://{0}:{1}@{2}/{3}?charset=utf8'.format(user, password, host, dbname)
	...
	for number, stock in stocks.items():
	    ...
	    stmt = select([latest_stock_info]).where(latest_stock_info.c.number == number)
		...
		try:
		    ...
		    logger.error('Failed to update stock info: %s, %s', e, stock)
