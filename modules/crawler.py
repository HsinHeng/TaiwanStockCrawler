...
class Crawler(object):
	...
	while idx < length:
		...
		logger.warning('(X) Timeout %s', url)
		    ...
		    if msgArray:
			logger.debug('(V) Retrieve %s/%s from %s', len(msgArray), self.count, url)
            	        msgArrays.extend(msgArray)
		    else:
			logger.warning('(X) Retrieve %s/%s from %s', len(msgArray), self.count, url)
		else:
	...
	for row in msgArrays:
		...
		'record_time': self.__to_datetime(row.get('tlong'), row.get('d'), row.get('t')),
